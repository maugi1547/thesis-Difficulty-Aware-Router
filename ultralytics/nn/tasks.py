...
def hook_router_to_head(model):
    ...
    for m in model.modules():
        if (
            m.__class__.__name__ == "DifficultyAwareRouter"
            or m.__class__.__name__ == "LightWeightDifficultyAwareRouter"
        ):
            router_module = m
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # --- TAMBAHAN UNTUK MOE-P2 ROUTER ---
            ...
            m is DifficultyAwareRouter
            or m is LightWeightDifficultyAwareRouter
            or m is UltraLightWeightDifficultyAwareRouter
        ):
