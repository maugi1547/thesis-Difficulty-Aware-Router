...
class DifficultyAwareRouter(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor:
        ...
        if self.training:
            ...
            # 🚨 MODIFIKASI KRUSIAL UNTUK TENSORRT:
            # Ambil elemen mentah sebagai 0D Boolean Tensor, BUKAN menggunakan .sum() > 0
            # TensorRT IIfConditional mewajibkan condition adalah skalar.
...
class UltraLightWeightDifficultyAwareRouter(nn.Module):
    ...
    def __init__(
        ...
        # =========================================================================
        # 1. JALUR UTAMA INFERENSI (COMPUTE-BOUND, SANGAT CEPAT DI TENSORRT)
        # =========================================================================
        ...
        # =========================================================================
        # 2. AUXILIARY BRANCH (HANYA UNTUK TRAINING - MENYUPLAI ROUTER LOSS)
        # =========================================================================
        self.proxy_pool = nn.MaxPool2d(kernel_size=4, stride=4)
        hidden_c = max(c_p3 // 8, 16) 
        ...
        # =========================================================================
        # 4. TRACKING STATE (STATELESS SAFE)
        # =========================================================================
        self.current_epoch = 0
        self._is_warmup = True
        self.current_activation_prob = torch.tensor(0.0)
        ...
        # Variabel yang akan DIBACA oleh fungsi compute_router_loss di loss.py
        self.last_entropy = torch.tensor(0.0)
        self.last_conf = torch.tensor(0.0)
        self.last_var = torch.tensor(0.0)
    ...
    def forward(self, x: list) -> torch.Tensor:
        ...
        z_fused = z_p3 + z_p2
        z_pool = self.gap(z_fused).view(B, -1)
        ...
        # =====================================================================
            ...
            # 🚨 Proxy head / Rumus Entropy SAMA SEKALI TIDAK DIJALANKAN!
            # CPU/GPU tidak akan tersiksa oleh Memory Bound!
...
class UltraLightWeightDifficultyAwareRouter(nn.Module):
    """ULTRA-LIGHT ROUTER (TENSORRT OPTIMIZED). --------------------------------------------------------------------- 1.
    100% Fully Convolutional (Tanpa nn.Linear). 2. Dimensi 4D dipertahankan dari awal hingga akhir (Tanpa Flatten
    memory). 3. Bebas AdaptiveAvgPool2d (Menggunakan Native Spatial Mean).
    """
    ...
    def _topk_spatial_pool(self, z: torch.Tensor, k: int = 10, alpha: float = 0.7) -> torch.Tensor:
        ...
        Args:
            ...
            k: jumlah lokasi spasial teratas yang diambil (default 10, sama dgn K=10 di _compute_stats proxy branch
                Anda)
            alpha: bobot komponen top-k vs mean global (0.7/0.3, sama dgn proxy)

        Returns:
            (B, C, 1, 1) — shape identik dengan output mean(dim=[2,3], keepdim=True): yang digantikan, jadi classifier
                di bawahnya TIDAK perlu diubah.
        """
        B, C, H, W = z.shape
        spatial_size = H * W
    ...
    def forward(self, x: list) -> torch.Tensor:
        ...
        z_p2 = self.squeeze_p2(f_p2_back.detach())
        z_fused = z_p3 + z_p2
        z_pool = self._topk_spatial_pool(
            z_fused,
            k=getattr(self, "gate_topk", 10),
