...
def load_and_patch_model(pt_path, use_cuda=False):
    ...
    # ========================================================
    # 🚨 PATCH: SUNTIKAN KOMPATIBILITAS UNTUK CHECKPOINT LLAMA
    # ========================================================
...
def cmd_check():
    ...
    with torch.no_grad():
        ...
        router.compute_gate(f_p3_neck, f_p2_back)
    ...
    print(
        f"  {'✓' if has_stats else '⚠'} running_mean: {router._running_mean if has_stats else 'None (akan pakai zeros)'}"
    )
    ...
    router_keys = [
        "model.16.sam.conv.weight",
        "model.16.conv_hint.weight",
        "model.16.layer_norm.weight",
        "model.16.layer_norm.bias",
        "model.16.mlp.0.weight",
        "model.16.mlp.0.bias",
        "model.16.mlp.2.weight",
        "model.16.mlp.2.bias",
...
def conv_bn_silu(net, x, W, prefix, stride=1):
    ...
    sc = net.add_scale_nd(x, trt.ScaleMode.CHANNEL, tw(shift), tw(scale), tw(np.ones_like(scale)), channel_axis=1)
...
def c2f_block(net, x, W, prefix):
    ...
    y1 = net.add_slice(x, trt.Dims([0, 0, 0, 0]), trt.Dims([1, half, H, Ww]), trt.Dims([1, 1, 1, 1])).get_output(0)
    y2 = net.add_slice(x, trt.Dims([0, half, 0, 0]), trt.Dims([1, half, H, Ww]), trt.Dims([1, 1, 1, 1])).get_output(0)
...
def sam_block(net, x, W, prefix):
    ...
    cm = net.add_concatenation([avg, mx])
    cm.axis = 1
...
def layer_norm_manual(net, x, W, prefix, C):
    ...
    veps = net.add_elementwise(var, const(net, np.array([[eps]], np.float32)), trt.ElementWiseOperation.SUM).get_output(
        0
    )
...
def fc_layer(net, x, W, prefix):
    """Pengganti add_fully_connected di TRT 10 menggunakan Konvolusi 1x1. Input x dijamin memiliki shape (B, C, 1, 1)
    ...
    cv = net.add_convolution_nd(input=x, num_output_maps=out_ch, kernel_shape=(1, 1), kernel=tw(w_conv), bias=tw(b))
    cv.stride_nd = (1, 1)
    cv.padding_nd = (0, 0)
...
def build_gate(net, f_p3_neck, f_p2_back, W):
    ...
    # Z_in & MLP
    zc = net.add_concatenation([z_vis, z_low, stats])
    zc.axis = 1
...
def build_if_conditional(net, cond_obj, p3_in, p2_in, W):
    """True branch = C2f P2. Akan menjadi "lazy evaluated" oleh TensorRT karena dependent terhadap
    IIfConditionalInputLayer (p3_in, p2_in). False branch = const tensor (dieksekusi "eager", tapi karena hanya
    const, 0 cost).
    """
    # ---- TRUE BRANCH: C2f P2 ----
...
def build_detect_head(net, W, feats):
    ...
    # ✅ PERBAIKAN: Gunakan first_transpose dengan tuple standard
    ...
    cv_dfl = net.add_convolution_nd(
        input=r_soft, num_output_maps=1, kernel_shape=(1, 1), kernel=tw(dfl_w), bias=tw(np.zeros(1, np.float32))
    )
...
def build_network(net, W):
    ...
    # GATE COMPUTATION
    _gate_2d, gate_bool = build_gate(net, f_p3_neck, f_p2_back, W)
...
def cmd_validate():
    ...
    if np.std(lats) > 0.05:
        ...
        print("   Hypothesis tesis terbukti: satu engine, latensi adaptif")
    else:
